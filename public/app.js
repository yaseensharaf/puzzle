...
// Wraps fetch with an AbortController so hung requests don't block forever.
...
// Checks the Content-Type before calling .json() to surface clear errors when
...
async function savePuzzleToServer(snapshotCanvas, stats) {
  try {
    ...
    const response = await fetchWithTimeout("/api/puzzles", {
      method: "POST",
      body: formData,
    });
 ...
 *
 * Also guards against duplicate entries when the user retries a failed load
 * by comparing incoming record IDs against what is already shown.
 */
async function loadGalleryFromServer() {
  hideGalleryLoadError();
  if (galleryRetryBtn) galleryRetryBtn.disabled = true;
  try {
    const response = await fetchWithTimeout("/api/puzzles");
    if (!response.ok) {
    ...
    // Skip records already shown to prevent duplicates on retry.
    const existingIds = new Set(galleryEntries.filter(e => e.id !== null).map(e => e.id));
    ...
    for (const record of newRecords.slice().reverse()) {
    ...
    if (galleryRetryBtn) galleryRetryBtn.disabled = false;
  }
}
 ...
 *
 * The Reset button is disabled for the duration of the server request to
 * prevent duplicate clicks from firing concurrent DELETE calls.
 */
async function resetEverything() {
  if (resetAllBtn) resetAllBtn.disabled = true;
  let serverCleared = true;
  try {
    const response = await fetchWithTimeout("/api/puzzles", { method: "DELETE" });
    if (!response.ok) {
      throw new Error(`Server responded with status ${response.status}`);
    }
  } catch (err) {
...
function drawBoardAndPieces() {
  ...
  // Two-pass draw: render all non-dragging pieces first, then the dragged
  // piece on top. Avoids allocating a sorted array copy on every animation frame.
  let draggingPiece = null;
  for (const piece of puzzle.pieces) {
    if (piece.dragging) { draggingPiece = piece; continue; }
    ctx.save();
  ...
  if (draggingPiece) {
    ...
    ctx.strokeStyle = "rgba(234,229,214,0.5)";
    ctx.lineWidth = 3;
...
// Guards against starting a second render loop if boot() is retried after a
// successful start (e.g. camera already running when the user clicks Retry).
...
async function boot() {
  ...
  try {
    ...
    // Guard: only start one render loop even if boot() is called again on retry.
